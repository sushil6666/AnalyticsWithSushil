...
compiled SQL, tests, and dependencies. `catalog.json` contained warehouse
metadata such as relation columns and data types.
...
These commands read project metadata from Parquet without querying Snowflake.
The files are written under `target/info_schema/v1/`.
...
The strict build already created the metadata index. `--no-compile` keeps its
column-level lineage. `--output-dir` creates a self-contained site that is safer
to publish than the full `target/` directory.
...
Open `dbt_v2_docs_daily_summary`, then review its descriptions, tests, compiled
SQL, model lineage, and column lineage for `total_order_amount`.
...
3. Run strict static analysis before docs generation when column lineage is required.
