...
> This reads model information from the dbt Information Schema. It does not need
> to query Snowflake for these results.

Optional lineage check:
...
> The strict build already created the metadata index. We use `--no-compile` so
> the generated site keeps the column lineage information. We use `--output-dir`
> to create a self-contained directory for publishing.

Show the validated dbt v2.0.6 structure:
